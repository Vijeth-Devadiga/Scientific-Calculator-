...
#     global expression
#     try:
#         # eval on expression built from math functions/constants like sin(, pi, etc.
...
# # ---------- display ----------
# display = tk.Entry(root, textvariable=display_var, font=("Helvetica", 20, "bold"),
#                    bg="white", fg="black", bd=4, relief="sunken", justify="right")
# display.grid(row=0, column=0, columnspan=7, sticky="nsew", padx=5, pady=5, ipady=10)

# # allow rows/columns to expand evenly
# for r in range(0, 12):
...
#     b = tk.Button(root, text=text, font=("Helvetica", 18), bg="#000000", fg="white",
...
# btn("log(", 3, 4, cmd=lambda: press("log("))    # natural log use log(x) for ln
# btn("ln(", 3, 5, cmd=lambda: press("log("))     # convenience duplicate
...
# # Row 5
# btn("0", 5, 0, colspan=2, cmd=lambda: press("0"))  # wider zero
...
# btn("sqrt_alt", 8, 6, cmd=lambda: press("sqrt("))  # alternative label

# # ---------- extra utilities (row 9) ----------
...
# # ---------- bottom row: large equal ----------
# btn("C", 10, 0, cmd=clear_all)
# btn("=", 10, 1, colspan=6, cmd=evaluate)  # large equal spanning many columns
